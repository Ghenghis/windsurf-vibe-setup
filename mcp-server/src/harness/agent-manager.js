...
class AgentManager {
  ...
  async createInitScript(features) {
    ...
    const hasReact = features.some(
      f => f.description.toLowerCase().includes('react') || f.keywords.includes('react')
    );

    const hasNode = features.some(
      f =>
        f.description.toLowerCase().includes('node') || f.description.toLowerCase().includes('api')
    );
...
`;
  ...
  async callLLM(prompt) {
    ...
    try {
      const response = await llmClients.callOllama('qwen2.5-coder:32b', prompt, {
        temperature: 0.7,
      });
