...
class AnthropicHarness extends EventEmitter {
  constructor(config = {}) {
    ...
    this.config = {
      ...
      targetTestPassRate:
        parseFloat(process.env.HARNESS_TARGET_PASS_RATE) || config.targetTestPassRate || 0.95,
      regressTestCount:
        parseInt(process.env.HARNESS_REGRESSION_TEST_COUNT) || config.regressTestCount || 5,
      contextWindowLimit: config.contextWindowLimit || 100000,
      checkpointInterval:
        parseInt(process.env.HARNESS_CHECKPOINT_INTERVAL) || config.checkpointInterval || 10,
      ...config,
    };
  ...
  async enforceSubscriptionOnly() {
    ...
    const blockedVars = ['ANTHROPIC_API_KEY', 'OPENAI_API_KEY', 'CLAUDE_API_KEY', 'API_KEY'];
  ...
  async initialize(projectSpec) {
    ...
    await fs.writeFile(path.join(this.projectDir, 'app-spec.md'), this.artifacts.appSpec);
...
*.tmp
  ...
  async runCodingSession() {
    ...
    const recentFeatures = features.filter(f => f.passes).slice(-this.config.regressTestCount);
    ...
    // Commit changes
    await this.commitProgress(`Session ${this.currentSession}: ${nextFeature.name}`);
  ...
  async checkHealth() {
    ...
    if (this.metrics.testsPassingRate >= this.config.targetTestPassRate) {
      console.log(
        `✅ Target pass rate achieved (${(this.config.targetTestPassRate * 100).toFixed(1)}%)`
      );
      await this.stop();
      return;
    }
  ...
  async checkpoint() {
    ...
    const checkpointPath = path.join(this.projectDir, `checkpoint-${this.currentSession}.json`);
  ...
  async generateFinalReport() {
    ...
    const runtime = this.startTime ? ((Date.now() - this.startTime) / 3600000).toFixed(1) : 0;
...
${features
  .filter(f => f.passes)
  .map(f => `✅ ${f.name} (Session ${f.session})`)
  .join('\n')}
...
## Session Timeline
${progress.sessions?.map(s => `- Session ${s.session} (${s.type}): ${s.summary}`).join('\n')}
...
`;
    ...
    await fs.writeFile(path.join(this.projectDir, 'HARNESS_REPORT.md'), report);
