...
class OpenInterpreterHarnessAdapter extends EventEmitter {
  ...
  async performBrowserAction(action) {
    try {
      ...
      switch (action.type) {
        case 'navigate':
          result = await this.interpreter.chat(`Navigate to URL: ${action.url}`);
          break;
        ...
        default:
          result = await this.interpreter.chat(`Perform browser action: ${JSON.stringify(action)}`);
      }
  ...
  parseTestResults(output) {
    ...
    const failureIndicators = ['failed', 'error', 'failing', '✗', '✖', 'fail:', 'failures:'];
