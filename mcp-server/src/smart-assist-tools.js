...
const explain_code = {
  ...
  handler: async ({ code, level = 'beginner', focus }) => {
    try {
      ...
      if (code.includes('function') || code.includes('=>')) language = 'JavaScript';
      ...
      else if (code.includes('{') && code.includes(':')) language = 'CSS/JSON';
      ...
      const concepts = [];
      if (code.includes('if') || code.includes('else')) concepts.push('Decision making (if/else)');
      if (code.includes('for') || code.includes('while')) concepts.push('Repetition (loops)');
      if (code.includes('function') || code.includes('def ')) concepts.push('Reusable actions (functions)');
      if (code.includes('return')) concepts.push('Giving back a result');
      if (code.includes('async') || code.includes('await')) concepts.push('Waiting for things (async)');
      if (code.includes('try') || code.includes('catch')) concepts.push('Error handling');
      if (code.includes('class')) concepts.push('Blueprints for objects (classes)');
      if (code.includes('import') || code.includes('require')) concepts.push('Using external tools');
